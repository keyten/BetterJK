...
void R_Register( void )
{
	...
	r_colorGrading = ri.Cvar_Get( "r_colorGrading", "1", CVAR_ARCHIVE, "Color grading with a 3D LUT: 0 = off, 1 = on, 2 = split screen, original on the left" );
	...
	r_debugAO = ri.Cvar_Get( "r_debugAO", "0", CVAR_CHEAT, "AO debug view: 1 = legacy SSAO, 2 = raw GTAO, 3 = denoised GTAO, 4 = reconstructed normals, 5 = linear depth, 6 = contact shadows, 7 = cascade shadows, 8 = cascade * contact shadows, 9 = final ambient visibility, 10 = AO map used by lighting" );
	...
	r_weatherWetRoughness = ri.Cvar_Get( "r_weatherWetRoughness", "1", CVAR_ARCHIVE, "Scale of the per material wet smoothing (1 = material table, 0 = none)" );
	r_weatherWetDarkening = ri.Cvar_Get( "r_weatherWetDarkening", "1", CVAR_ARCHIVE, "Scale of the per material wet albedo darkening (1 = material table)" );
	...
	r_puddleSlope = ri.Cvar_Get( "r_puddleSlope", "0.90 0.98", CVAR_ARCHIVE, "Geometric normal z range where puddles fade in: min max" );
	r_puddleScale = ri.Cvar_Get( "r_puddleScale", "192", CVAR_ARCHIVE, "World size of the puddle noise pattern" );
	r_puddleHeight = ri.Cvar_Get( "r_puddleHeight", "1", CVAR_ARCHIVE, "Puddles follow the height map of parallax (normalHeightMap) materials: water fills cracks and low areas first" );
	r_puddleHeightSoftness = ri.Cvar_Get( "r_puddleHeightSoftness", "0.08", CVAR_ARCHIVE, "Water line transition width of height aware puddles, in 0..1 material height" );
	r_puddleHeightFill = ri.Cvar_Get( "r_puddleHeightFill", "0", CVAR_ARCHIVE, "Static water level bias of height aware puddles: > 0 floods more of the relief, < 0 only the deepest cracks" );
	r_puddleRipples = ri.Cvar_Get( "r_puddleRipples", "1", CVAR_ARCHIVE, "Procedural rain ripples on puddles (normal only, needs r_weatherPuddles and rain)" );
	r_puddleRippleStrength = ri.Cvar_Get( "r_puddleRippleStrength", "0.35", CVAR_ARCHIVE, "Peak slope of a puddle ripple ring (0..2)" );
	r_puddleRippleScale = ri.Cvar_Get( "r_puddleRippleScale", "20", CVAR_ARCHIVE, "World size of a ripple cell, a ring grows to 0.3 x this radius" );
	r_puddleRippleRate = ri.Cvar_Get( "r_puddleRippleRate", "1.1", CVAR_ARCHIVE, "Ripple rings per second per cell" );
	...
	r_skinSSSMixed = ri.Cvar_Get( "r_skinSSSMixed", "0", CVAR_ARCHIVE | CVAR_LATCH, "Skin scattering also on *_head textures, which mix hair, ears and neck in one texture (vid_restart)" );
	...
	r_autoGrass = ri.Cvar_Get( "r_autoGrass", "0", CVAR_ARCHIVE,
	...
	r_autoGrassDebug = ri.Cvar_Get( "r_autoGrassDebug", "0", CVAR_CHEAT,
	...
	r_autoGrassLodDist = ri.Cvar_Get( "r_autoGrassLodDist", "600", CVAR_ARCHIVE,
	...
	r_foliageInteractionMax = ri.Cvar_Get( "r_foliageInteractionMax", "8", CVAR_ARCHIVE,
	...
	r_foliageInteractionNPC = ri.Cvar_Get( "r_foliageInteractionNPC", "1", CVAR_ARCHIVE,
	...
	r_glslCache = ri.Cvar_Get( "r_glslCache", "1", CVAR_ARCHIVE | CVAR_LATCH, "Keep compiled GLSL programs in glslcache/ so later starts skip compiling them (needs GL_ARB_get_program_binary)" );
	ri.Cvar_CheckRange( r_glslCache, 0, 1, qtrue );
	r_glslCacheMaxMB = ri.Cvar_Get( "r_glslCacheMaxMB", "512", CVAR_ARCHIVE, "Size limit of the GLSL program cache file, least recently used programs are dropped first" );
	...
	r_volumetricFogHeightOpaque = ri.Cvar_Get("r_volumetricFogHeightOpaque", "3000", CVAR_ARCHIVE, "Froxel fog height fog: distance at which the medium at the base height becomes opaque, as fogParms depthForOpaque (world units)");
	...
	r_pomSelfShadowLights = ri.Cvar_Get( "r_pomSelfShadowLights", "1", CVAR_ARCHIVE, "POM self shadowed dynamic lights: 0 sun only, 1 sun + strongest local light, 2 sun + strongest r_pomSelfShadowMaxLocalLights, 3 all lights" );
	ri.Cvar_CheckRange( r_pomSelfShadowLights, 0, 3, qtrue );
	r_pomSelfShadowMaxLocalLights = ri.Cvar_Get( "r_pomSelfShadowMaxLocalLights", "2", CVAR_ARCHIVE, "Local lights with POM self shadow per pixel for r_pomSelfShadowLights 2" );
