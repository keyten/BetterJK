...
cvar_t  *r_puddleRoughness;
cvar_t  *r_puddleSlope;
cvar_t  *r_puddleScale;
cvar_t  *r_puddleHeight;
cvar_t  *r_puddleHeightSoftness;
cvar_t  *r_puddleHeightFill;
cvar_t  *r_puddleRipples;
cvar_t  *r_puddleRippleStrength;
cvar_t  *r_puddleRippleScale;
cvar_t  *r_puddleRippleRate;
cvar_t  *r_ssrQuality;
cvar_t  *r_ssrSteps;
...
void R_Register( void ) 
{
	...
	r_rainOpacity = ri_Cvar_Get_NoComm("r_rainOpacity", "1", CVAR_ARCHIVE,
		"r_rainStreaks: rain opacity scale");
	r_rainLighting = ri_Cvar_Get_NoComm("r_rainLighting", "1", CVAR_ARCHIVE,
		"r_rainStreaks: 0 fixed tint (legacy brightness), 1 lit by the light grid and the sun");
	r_rainDebug = ri_Cvar_Get_NoComm("r_rainDebug", "0", CVAR_CHEAT,
	...
	r_debugAO = ri_Cvar_Get_NoComm( "r_debugAO", "0", CVAR_CHEAT, "AO debug view: 1 = legacy SSAO, 2 = raw GTAO, 3 = denoised GTAO, 4 = reconstructed normals, 5 = linear depth, 6 = contact shadows, 7 = cascade shadows, 8 = cascade * contact shadows, 9 = final ambient visibility, 10 = AO map used by lighting" );
	ri.Cvar_CheckRange( r_debugAO, 0, 10, qtrue );
	...
	r_weatherWetStrength = ri_Cvar_Get_NoComm( "r_weatherWetStrength", "1", CVAR_ARCHIVE, "Wetness of fully rain exposed surfaces, 0-1" );
	r_weatherWetRoughness = ri_Cvar_Get_NoComm( "r_weatherWetRoughness", "1", CVAR_ARCHIVE, "Scale of the per material wet smoothing (1 = material table, 0 = none)" );
	r_weatherWetDarkening = ri_Cvar_Get_NoComm( "r_weatherWetDarkening", "1", CVAR_ARCHIVE, "Scale of the per material wet albedo darkening (1 = material table)" );
	r_weatherWetNormal = ri_Cvar_Get_NoComm( "r_weatherWetNormal", "1", CVAR_ARCHIVE, "Scale of the per material wet normal flattening (1 = material table)" );
	r_weatherWetEntityFacing = ri_Cvar_Get_NoComm( "r_weatherWetEntityFacing", "0.85", CVAR_ARCHIVE, "Wetness of vertical faces of characters and props (world walls: 0.5)" );
	r_weatherWetBias = ri_Cvar_Get_NoComm( "r_weatherWetBias", "2", CVAR_ARCHIVE, "Rain occlusion depth bias in world units" );
	r_weatherWetnessDebug = ri_Cvar_Get_NoComm( "r_weatherWetnessDebug", "0", CVAR_CHEAT, "1 rain exposure, 2 wetness mask (magenta: excluded), 3 effective roughness, 4 dry / wet split, 5 puddle slope, 6 puddle noise, 7 exposure x slope, 8 puddle mask, 9 puddle roughness, 10 puddle eligibility, 11 material height (magenta: none), 12 height low / high, 13 macro puddle mask, 14 micro depression mask, 15 combined puddle, 16 wet material class, 17 ripple height, 18 ripple normal offset, 19 ripple masking, 20 final normal" );
	...
	r_puddleCoverage = ri_Cvar_Get_NoComm( "r_puddleCoverage", "0.35", CVAR_ARCHIVE, "Fraction of flat exposed area covered by puddles, 0-1" );
	r_puddleRoughness = ri_Cvar_Get_NoComm( "r_puddleRoughness", "0.04", CVAR_ARCHIVE, "Roughness of the puddle water surface" );
	r_puddleSlope = ri_Cvar_Get_NoComm( "r_puddleSlope", "0.90 0.98", CVAR_ARCHIVE, "Geometric normal z range where puddles fade in: min max" );
	r_puddleScale = ri_Cvar_Get_NoComm( "r_puddleScale", "192", CVAR_ARCHIVE, "World size of the puddle noise pattern" );
	r_puddleHeight = ri_Cvar_Get_NoComm( "r_puddleHeight", "1", CVAR_ARCHIVE, "Puddles follow the height map of parallax (normalHeightMap) materials: water fills cracks and low areas first" );
	r_puddleHeightSoftness = ri_Cvar_Get_NoComm( "r_puddleHeightSoftness", "0.08", CVAR_ARCHIVE, "Water line transition width of height aware puddles, in 0..1 material height" );
	r_puddleHeightFill = ri_Cvar_Get_NoComm( "r_puddleHeightFill", "0", CVAR_ARCHIVE, "Static water level bias of height aware puddles: > 0 floods more of the relief, < 0 only the deepest cracks" );
	r_puddleRipples = ri_Cvar_Get_NoComm( "r_puddleRipples", "1", CVAR_ARCHIVE, "Procedural rain ripples on puddles (normal only, needs r_weatherPuddles and rain)" );
	r_puddleRippleStrength = ri_Cvar_Get_NoComm( "r_puddleRippleStrength", "0.35", CVAR_ARCHIVE, "Peak slope of a puddle ripple ring (0..2)" );
	r_puddleRippleScale = ri_Cvar_Get_NoComm( "r_puddleRippleScale", "20", CVAR_ARCHIVE, "World size of a ripple cell, a ring grows to 0.3 x this radius" );
	r_puddleRippleRate = ri_Cvar_Get_NoComm( "r_puddleRippleRate", "1.1", CVAR_ARCHIVE, "Ripple rings per second per cell" );
	...
	r_ssrStrength = ri_Cvar_Get_NoComm( "r_ssrStrength", "1", CVAR_ARCHIVE, "SSR confidence scale: 0 = cubemap reflections only, 1 = full SSR where it is reliable" );
	...
	r_ssgiHalfResolution = ri_Cvar_Get_NoComm( "r_ssgiHalfResolution", "-1", CVAR_ARCHIVE, "SSGI rays at half resolution: -1 = from r_ssgiQuality, 0 = full, 1 = half (applied at vid_restart)" );
	...
	r_dynamicShadowMaxLights = ri_Cvar_Get_NoComm( "r_dynamicShadowMaxLights", "4", CVAR_ARCHIVE, "Forward+: dynamic lights with a shadow cube (needs r_dlightMode 2), the most important ones get them" );
	...
	r_ltcAutoAreaLights = ri_Cvar_Get_NoComm( "r_ltcAutoAreaLights", "1", CVAR_ARCHIVE, "LTC area lights from the emissive surfaces of maps without an .arealights.json (at map load, r_reloadAreaLights): 0 off, 1 confident lamp shapes, 2 also loosely fitted ones (glow / emissive / surfacelight surfaces)" );
	r_saberAreaLights = ri_Cvar_Get_NoComm( "r_saberAreaLights", "0", CVAR_ARCHIVE, "Saber blades light as LTC lines instead of a point light (requires r_ltcAreaLights 1)" );
	...
	r_volumetricFogAnisotropy = ri_Cvar_Get_NoComm("r_volumetricFogAnisotropy", "0.2", CVAR_ARCHIVE, "Froxel fog: Henyey-Greenstein g of the sun and dynamic light scattering, 0 = isotropic, > 0 forward, < 0 backward");
	...
	r_volumetricFogHeightOpaque = ri_Cvar_Get_NoComm("r_volumetricFogHeightOpaque", "3000", CVAR_ARCHIVE, "Froxel fog height fog: distance at which the medium at the base height becomes opaque, as fogParms depthForOpaque (world units)");
	ri.Cvar_CheckRange(r_volumetricFogHeightOpaque, 1.0f, 1000000.0f, qfalse);
	r_volumetricFogHeightBase = ri_Cvar_Get_NoComm("r_volumetricFogHeightBase", "0", CVAR_ARCHIVE, "Froxel fog height fog: world Z of the base height, set to the lowest floor of the map when it loads");
	...
	r_volumetricFogHeightMax = ri_Cvar_Get_NoComm("r_volumetricFogHeightMax", "1", CVAR_ARCHIVE, "Froxel fog height fog: maximum density below the base height, as a multiple of the base density");
	...
	r_pomSelfShadowLights = ri_Cvar_Get_NoComm( "r_pomSelfShadowLights", "1", CVAR_ARCHIVE, "POM self shadowed dynamic lights: 0 sun only, 1 sun + strongest local light, 2 sun + strongest r_pomSelfShadowMaxLocalLights, 3 all lights" );
	ri.Cvar_CheckRange( r_pomSelfShadowLights, 0, 3, qtrue );
	r_pomSelfShadowMaxLocalLights = ri_Cvar_Get_NoComm( "r_pomSelfShadowMaxLocalLights", "2", CVAR_ARCHIVE, "Local lights with POM self shadow per pixel for r_pomSelfShadowLights 2" );
	...
	r_pomSilhouette = ri_Cvar_Get_NoComm( "r_pomSilhouette", "0", CVAR_ARCHIVE | CVAR_LATCH, "Silhouette parallax occlusion mapping for materials with the silhouettePOM keyword (needs r_parallaxMapping 1)" );
